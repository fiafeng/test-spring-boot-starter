package com.fiafeng.flowable.flowable.service.impl;
...
import com.fiafeng.flowable.flowable.common.constant.ProcessConstants;
import com.fiafeng.flowable.flowable.common.enums.FlowComment;
import com.fiafeng.flowable.flowable.domain.SysForm;
import com.fiafeng.flowable.flowable.domain.dto.FlowCommentDto;
import com.fiafeng.flowable.flowable.domain.dto.FlowNextDto;
import com.fiafeng.flowable.flowable.domain.dto.FlowTaskDto;
import com.fiafeng.flowable.flowable.domain.dto.FlowViewerDto;
import com.fiafeng.flowable.flowable.domain.vo.FlowQueryVo;
import com.fiafeng.flowable.flowable.domain.vo.FlowTaskVo;
import com.fiafeng.flowable.flowable.factory.FlowServiceFactory;
import com.fiafeng.flowable.flowable.flow.FindNextNodeUtil;
import com.fiafeng.flowable.flowable.flow.FlowableUtils;
import com.fiafeng.flowable.flowable.service.IFlowTaskService;
import com.fiafeng.flowable.flowable.service.ISysDeployFormService;
import com.fiafeng.flowable.flowable.service.ISysFormService;
...
public class FlowTaskServiceImpl extends FlowServiceFactory implements IFlowTaskService {
    ...
    public AjaxResult complete(FlowTaskVo taskVo) {
        ...
        if (DelegationState.PENDING.equals(task.getDelegationState())) {
            ...
            Long userId = tokenService.getLoginUser().getUser().getId();
        ...
        for (HistoricProcessInstance hisIns : historicProcessInstances) {
            ...
            if (CollectionUtils.isNotEmpty(taskList)) {
                ...
                if (StringUtils.isNotBlank(taskList.get(0).getAssignee())) {
                    // 当前任务节点办理人信息
                    IBaseUser sysUser = sysUserService.selectUserByUserId(Long.parseLong(taskList.get(0).getAssignee()));
...
//                        StringUtils.isBlank(flowTaskVo.getComment()) ? "取消申请" : flowTaskVo.getComment());
    ...
    public AjaxResult revokeProcess(FlowTaskVo flowTaskVo) {
        ...
        IBaseUser loginUser = tokenService.getLoginUser().getUser();
        ...
        for (HistoricTaskInstance hti : htiList) {
            if (loginUser.getId().toString().equals(hti.getAssignee())) {
                myTaskId = hti.getId();
                myTask = hti;
                break;
            }
...
//      Map<String, VariableInstance> variables = runtimeService.getVariableInstances(currentTask.getExecutionId());
    ...
    public AjaxResult todoList(FlowQueryVo queryVo) {
        Page<FlowTaskDto> page = new Page<>();
        // 只查看自己的数据
        IBaseUser sysUser = tokenService.getLoginUser().getUser();
        TaskQuery taskQuery = taskService.createTaskQuery()
                .active()
                .includeProcessVariables()
                .taskCandidateGroupIn(userRoleService.queryUserRoleListByUserId(sysUser.getId()).stream().map(role -> role.getId().toString()).collect(Collectors.toList()))
                .taskCandidateOrAssigned(sysUser.getId().toString())
...
//        }
        ...
        for (Task task : taskList) {
            ...
            HistoricProcessInstance historicProcessInstance = historyService.createHistoricProcessInstanceQuery()
                    .processInstanceId(task.getProcessInstanceId())
                    .singleResult();
            IBaseUser startUser = sysUserService.selectUserByUserId(Long.parseLong(historicProcessInstance.getStartUserId()));
...
//                } else
                if (StringUtils.isNotBlank(histIns.getTaskId())) {
                    ...
                    if (StringUtils.isNotBlank(histIns.getAssignee())) {
                        IBaseUser sysUser = sysUserService.selectUserByUserId(Long.parseLong(histIns.getAssignee()));
                        flowTask.setAssigneeId(sysUser.getId());
                        flowTask.setAssigneeName(sysUser.getUsername());
                    ...
                    for (HistoricIdentityLink identityLink : linksForTask) {
                        ...
                        if ("candidate".equals(identityLink.getType())) {
                            if (StringUtils.isNotBlank(identityLink.getUserId())) {
                                IBaseUser sysUser = sysUserService.selectUserByUserId(Long.parseLong(identityLink.getUserId()));
                                stringBuilder.append(sysUser.getUsername()).append(",");
                            }
                            if (StringUtils.isNotBlank(identityLink.getGroupId())) {
                                IBaseRole sysRole = sysRoleService.queryRoleByRoleId(Long.parseLong(identityLink.getGroupId()));
