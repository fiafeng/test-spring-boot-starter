package com.fiafeng.flowable.flowable.controller;
...
import com.fiafeng.flowable.flowable.domain.SysExpression;
import com.fiafeng.flowable.flowable.domain.dto.FlowSaveXmlVo;
import com.fiafeng.flowable.flowable.service.IFlowDefinitionService;
...
//            @ApiParam(value = "流程定义id")
            @PathVariable(value = "deployId") String deployId, HttpServletResponse response) {
...
//            @ApiParam(value = "1:激活,2:挂起", required = true)
...
//            @ApiParam(value = "流程部署ID", required = true)
    ...
    @GetMapping("/userList")
    public AjaxResult userList(IBaseUser user) {
        List<IBaseUser> list = userService.selectUserListAll(user);
        return AjaxResult.success(list);
    }

    //    @ApiOperation(value = "指定流程办理组列表")
    @GetMapping("/roleList")
    public AjaxResult roleList(IBaseRole role) {
        List<IBaseRole> list = sysRoleService.queryRoleListAll();
        return AjaxResult.success(list);
    }
