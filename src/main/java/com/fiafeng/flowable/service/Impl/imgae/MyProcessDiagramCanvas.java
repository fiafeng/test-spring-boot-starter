...
public class MyProcessDiagramCanvas {
    ...
    protected static final Logger LOGGER = LoggerFactory.getLogger(MyProcessDiagramCanvas.class);
    ...
    protected static final int CONDITIONAL_INDICATOR_HEIGHT = 16;
    protected static final int DEFAULT_INDICATOR_WIDTH = 10;
    protected static final int MARKER_WIDTH = 12;
    protected static final int MARKER_HEIGHT = 12;
    ...
    protected static BufferedImage USER_TASK_IMAGE;
    protected static BufferedImage SCRIPT_TASK_IMAGE;
    protected static BufferedImage SERVICE_TASK_IMAGE;
    protected static BufferedImage RECEIVE_TASK_IMAGE;
    protected static BufferedImage SEND_TASK_IMAGE;
    protected static BufferedImage MANUAL_TASK_IMAGE;
     ...
     */
    public MyProcessDiagramCanvas(int width, int height, int minX, int minY, String imageType, String activityFontName, String labelFontName, ClassLoader customClassLoader) {
    ...
    /**
     * Creates an empty canvas with given width and height. Allows to specify minimal boundaries on the left and upper side of the canvas. This is useful for diagrams that have
     * white space there (e.g. Assignation). Everything beneath these minimum values will be cropped.
     *
...
//		LABEL_FONT = new Font(labelFontName, Font.ITALIC, 10);
        ...
        try {
            USER_TASK_IMAGE = ImageIO.read(ReflectUtil.getResource("org/flowable/icons/userTask.png", customClassLoader));
            SCRIPT_TASK_IMAGE = ImageIO.read(ReflectUtil.getResource("org/flowable/icons/scriptTask.png", customClassLoader));
            SERVICE_TASK_IMAGE = ImageIO.read(ReflectUtil.getResource("org/flowable/icons/serviceTask.png", customClassLoader));
            RECEIVE_TASK_IMAGE = ImageIO.read(ReflectUtil.getResource("org/flowable/icons/receiveTask.png", customClassLoader));
            SEND_TASK_IMAGE = ImageIO.read(ReflectUtil.getResource("org/flowable/icons/sendTask.png", customClassLoader));
            MANUAL_TASK_IMAGE = ImageIO.read(ReflectUtil.getResource("org/flowable/icons/manualTask.png", customClassLoader));
    ...
    public void drawCatchingEvent(GraphicInfo graphicInfo, boolean isInterrupting, BufferedImage image, String eventType, double scaleFactor) {
        ...
        Ellipse2D innerCircle = getInnerCircle(graphicInfo, scaleFactor);
    ...
    private static Ellipse2D getInnerCircle(GraphicInfo graphicInfo, double scaleFactor) {
        ...
        return new Ellipse2D.Double(innerCircleX, innerCircleY, innerCircleWidth, innerCircleHeight);
    }
    ...
    public void drawConnection(int[] xPoints, int[] yPoints, boolean conditional, boolean isDefault, String connectionType, AssociationDirection associationDirection,
        ...
        drawConnection(xPoints, yPoints, conditional, isDefault, connectionType, associationDirection, highLighted, scaleFactor, HIGHLIGHT_COLOR);
    }

    public void drawConnection(int[] xPoints, int[] yPoints, boolean conditional, boolean isDefault, String connectionType,
                               AssociationDirection associationDirection,
                               boolean highLighted, double scaleFactor, Color highLightedColor) {
    ...
    public void drawSequenceFlowWithoutArrow(int srcX, int srcY, int targetX, int targetY, boolean conditional, double scaleFactor) {
        drawSequenceFlowWithoutArrow(srcX, srcY, targetX, targetY, conditional, false, scaleFactor, HIGHLIGHT_COLOR);
    }

    public void drawSequenceFlowWithoutArrow(int srcX, int srcY, int targetX, int targetY, boolean conditional, double scaleFactor, Color highLightedColor) {
        drawSequenceFlowWithoutArrow(srcX, srcY, targetX, targetY, conditional, true, scaleFactor, highLightedColor);
    }

    public void drawSequenceFlowWithoutArrow(int srcX, int srcY, int targetX, int targetY, boolean conditional, boolean highLighted, double scaleFactor, Color highLightedColor) {
        Paint originalPaint = g.getPaint();
        if (highLighted) {
            g.setPaint(highLightedColor);
        }
    ...
    public void drawArrowHead(Line2D.Double line, double scaleFactor) {
        Polygon arrowHead = getPolygon(scaleFactor);
    ...
    private static Polygon getPolygon(double scaleFactor) {
    ...
    public void drawConditionalSequenceFlowIndicator(Line2D.Double line, double scaleFactor) {
        ...
        conditionalIndicator.addPoint(0, CONDITIONAL_INDICATOR_HEIGHT);
        conditionalIndicator.addPoint(halfOfHorizontal, halfOfVertical);
    ...
    public void drawCollapsedSubProcess(String name, GraphicInfo graphicInfo, Boolean isTriggeredByEvent) {
        drawCollapsedTask(name, graphicInfo, isTriggeredByEvent);
    }
    ...
    public void drawCollapsedMarker(int x, int y, int width, int height) {
        ...
        int rectangleHeight = MARKER_HEIGHT;
    ...
    public void drawEventBasedGateway(GraphicInfo graphicInfo, double scaleFactor) {
        ...
        if (scaleFactor == 1.0) {
            ...
            Polygon pentagon = getPentagon(width, x, y);
    ...
    private static Polygon getPentagon(int width, int x, int y) {
        double r = width / 6.;
     ...
     * @param height 高度
     */
    public void drawRunningActivitiHighLight(int x, int y, int width, int height, Color color) {
        Paint originalPaint = g.getPaint();
        Stroke originalStroke = g.getStroke();
    ...
    public void drawRunningActivitiHighLight(int x, int y, int width, int height) {
        drawRunningActivitiHighLight(x, y, width, height, RUNNING_HIGHLIGHT_COLOR);
    }

    public void drawTextAnnotation(String text, GraphicInfo graphicInfo) {
        drawTextAnnotation(text, graphicInfo, ANNOTATION_FONT);
    }

    public void drawTextAnnotation(String text, GraphicInfo graphicInfo, Font font) {
        int x = (int) graphicInfo.getX();
        int y = (int) graphicInfo.getY();
        int width = (int) graphicInfo.getWidth();
        int height = (int) graphicInfo.getHeight();
...
//        x += 0.5;
    ...
    public void drawLabel(String text, GraphicInfo graphicInfo, boolean centered) {
        drawLabel(text, graphicInfo, centered, LABEL_COLOR);
    }
     ...
     * @param centered    是否居中
     */
    public void drawLabel(String text, GraphicInfo graphicInfo, boolean centered, Color color) {
     ...
     *
     * @param sourceShapeType   源形状类型
     * @param targetShapeType   目标形状
     * @param sourceGraphicInfo 源画笔信息
     * @param targetGraphicInfo 目标画笔信息
     * @param graphicInfoList   画笔列表
     */
    public List<GraphicInfo> connectionPerfectionist(SHAPE_TYPE sourceShapeType, SHAPE_TYPE targetShapeType, GraphicInfo sourceGraphicInfo, GraphicInfo targetGraphicInfo,
    ...
    private static Path2D.Double getRhombus(GraphicInfo graphicInfo) {
